...
from nn import predict, initialize_parameters, compute_cost, update_parameters, \
...
def l_layer_model(X, Y, X_val, Y_val, layers_dims, learning_rate,
                  ...
                  use_batchnorm=False):
    """
    Trains an L-layer neural network with early stopping based on validation cost.
    Also tracks and prints training cost every 100 mini-batch steps.

    Returns:
        parameters (Dict): Trained network parameters
        val_costs (List[Tuple[int, float]]): Validation costs every 100 steps
        train_costs (List[Tuple[int, float]]): Training costs every 100 steps
    """
    ...
    while step < max_training_steps and no_improve_counts < max_no_improve_steps:
        ...
        for j in range(0, m, batch_size):
            ...
            # Every 100 steps → collect and print cost (train + val)
            if step % 100 == 0:
                # Training cost on current batch (for performance tracking)
                train_cost = compute_cost(AL, Y_batch)
                train_costs.append((step, train_cost))

                # Validation cost on full validation set
...
if __name__ == '__main__':
    ...
    learning_rate = 0.009
    batch_size = 128
    max_training_steps = 100000  # large upper bound to allow early stopping
    ...
    plt.ylabel('Cost')
    plt.title('Training & Validation Cost every 100 Steps')
    plt.grid(True)
    plt.legend()
    plt.savefig("training_validation_costs_batch_norm.png")
    plt.show()

    # Print all cost values for report
